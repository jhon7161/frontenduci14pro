...
const Hoja2 = () => {
  ...
  const { hoja1, hoja2, _id: pacienteId } = pacienteSeleccionado || {};
  const peso = parseFloat(hoja1?.peso) || 1;
  const {
    balances = {},
    administrados = [],
    eliminados = {},
  ...
  const {
    ...
    const totalBalanceDiaAdm = totals.reduce((acc, cur) => acc + cur.totalAdm, 0);
    const totalBalanceDiaElim = totals.reduce((acc, cur) => acc + cur.totalElim, 0);
    const balanceDia = totalBalanceDiaAdm - totalBalanceDiaElim;
    const diuresisTotal = eliminados.diuresisHora?.reduce((s, v) => s + (parseFloat(v) || 0), 0) || 0;
  ...
  }, [hoja2, peso, administrados, eliminados, otrosEliminados, balances, hours, getDosisArray]);


  // Handlers para las entradas de datos
  const handleDataChange = (path, value) => {
    dispatch(updateHojaLocal({
      hoja: "hoja2",
      datos: { ...setNestedValue(hoja2, path, value) },
    }));
  };
  
  // Función corregida para garantizar la inmutabilidad de los arrays y objetos
  ...
  const handleGuardar = async () => {
    if (!pacienteId) {
      ...
      dispatch(notifyWithTimeout('No se pudo guardar: ID de paciente no disponible', 4000));
  ...
  if (status === "failed" && !pacienteSeleccionado) {
  ...
  return (
    <>
      <div className="hoja2-container">
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th rowSpan="2">TOTAL ADM</th>
                <th rowSpan="2">TOTAL ACUM</th>
                <th colSpan={otrosEliminados.length > 0 ? otrosEliminados.length + 4 : 5}>ELIMINADOS</th>
                <th rowSpan="2">TOTAL HORA</th>
                <th rowSpan="2">ACUM</th>
                <th rowSpan="2">BALANCE HORA</th>
                <th rowSpan="2">ACUM</th>
              </tr>
              <tr>
                  ...
                  administrados.map((item, i) => (
                    <th key={i}>
                      <input
                        type="text"
                        value={item.nombre ?? ""}
                        onChange={(e) => handleListChange("administrados", "update", i, null, e.target.value)}
                        placeholder="Nombre"
                      />
                      <button className="btn--remove" onClick={() => handleListChange("administrados", "remove", i)}>
                        X
                      </button>
                    </th>
                  ))
                  ...
                  otrosEliminados.map((item, i) => (
                    <th key={i}>
                      <input
                        type="text"
                        value={item.nombre ?? ""}
                        onChange={(e) => handleListChange("eliminados.otros", "update", i, null, e.target.value)}
                        placeholder="Otro"
                      />
                      <button className="btn--remove" onClick={() => handleListChange("eliminados.otros", "remove", i)}>
                        X
                      </button>
                    </th>
                  ))
            ...
            <tbody>
              {hours.map((hour, hIdx) => {
                const t = totals[hIdx];
                const totalAdmAcum = totals.slice(0, hIdx + 1).reduce((s, t) => s + t.totalAdm, 0);
                const totalElimAcum = totals.slice(0, hIdx + 1).reduce((s, t) => s + t.totalElim, 0);
                const balanceAcum = totals.slice(0, hIdx + 1).reduce((s, t) => s + t.balanceHora, 0);
                  ...
                  <tr key={hour}>
                      ...
                      administrados.map((item, i) => (
                        <td key={i} data-label={item.nombre || "Administrado"}>
                          <input
                            ...
                            onChange={(e) => handleListChange("administrados", "update", i, hIdx, e.target.value)}
                            autoComplete="off"
                          />
                        </td>
                      ))
                    ...
                    <td data-label="Diuresis">
                      <input
                        ...
                        onChange={(e) => handleDataChange(`eliminados.diuresisHora[${hIdx}]`, e.target.value)}
                      />
                    </td>
                    ...
                    <td data-label="UF">
                      <input
                        ...
                        onChange={(e) => handleDataChange(`eliminados.uf[${hIdx}]`, e.target.value)}
                      />
                    </td>
                      ...
                      otrosEliminados.map((item, i) => (
                        <td key={i} data-label={item.nombre || "Otro Eliminado"}>
                          <input
                            ...
                            onChange={(e) => handleListChange("eliminados.otros", "update", i, hIdx, e.target.value)}
                            autoComplete="off"
                          />
                        </td>
                      ))
        ...
        <div className="auxiliares-balances-wrapper">
          <div className="auxiliares-table-container">
            <h3 className="section-title">Auxiliares de Enfermería</h3>
          ...
          <div className="balances-table-container">
            ...
            <table className="info-table">
              <tbody>
                <tr>
                  ...
                  <td data-label="Valor Previo">
                    <input
                      ...
                      onChange={(e) => handleDataChange("balances.previo", parseFloat(e.target.value) || 0)}
                    />
                  </td>
                </tr>
                <tr>
                  <td data-label="ELIM">ELIM</td>
                  <td data-label="Total Elim.">
                    <input
                      type="number"
                  ...
                  <td data-label="ACUM">ACUM</td>
                  <td data-label="Balance Acum.">
                    <input
                      type="number"
                ...
                <tr>
                  <td data-label="BAL.DIA">BAL.DIA</td>
                  <td data-label="Balance Día">
                    <input
                      type="number"
                  ...
                  <td data-label="GU">GU</td>
                  <td data-label="Gasto Urinario">
                    <input
                      type="number"
